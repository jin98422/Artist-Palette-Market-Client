...
export default function UploadArtwork(props) {
    ...
    useEffect(() => {
        if(CheckToken()) {
            ...
            setVendorDefault(decodedToken.first_name + " " + decodedToken.last_name)
            setAdmin(decodedToken.first_name + " " + decodedToken.last_name)
    ...
    const uploadSubmit = (e) => {
        ...
        if(CheckToken()) {      
            if (artFile === null) {
                setFileError(true);
                setFileErrorText("You must upload your artwork.")
                return;
            }
            setIsLoading(true);
            ...
            let newDate = `${event.getMonth() + 1}/${event.getDate()}/${event.getFullYear()}`;
            console.log(newDate)
            ...
            axios.post(`${window.$host}/artwork/upload`, formData, {
                headers: {
                ...
                if(res.status === 200) {
                    ...
                    setArtworkDate(new Date());
                    setSelectedTags("")
                    setDescription("")
                    setTitleDefault("")
                    setVendorDefault(admin)
                    setArtType("")
                    setVariants([
                        {
                            key: 0,
    ...
    return (
        <div>
            <Header openDialog={false} openDialog={true} title="Tell us About Your Artwork"/>
            ...
            <div id="upload-result" className={classes.paper}>
                ...
                <form className={classes.form} onSubmit={uploadSubmit}>
                    {isUploaded && <Alert severity="success">Upload Success! your artworks added on Palette Market</Alert>}
                    {isUploadFail && <Alert severity="error">Upload Failed!. Please try again.</Alert>}
                    ...
                    <Grid container spacing={2}>
                        ...
                        <Grid item xs={12} sm={12}>
                            <FormLabel className={classes.formLable} component="legend">Tell Us About Your Artwork</FormLabel>
                            <SunEditor
                                setOptions={{
                                    plugins: plugins,
                                    buttonList: [
                        ...
                        <Grid item xs={12} sm={6}>
                            <FormLabel className={classes.formLable} component="legend">Add Your Tags</FormLabel>
                            <Autocomplete
                                id="predefinded-tags"
                                onChange={
                                    (event, newValue) => {
                        ...
                        <Grid item xs={12} sm={6}>
                            <FormLabel className={classes.formLable} component="legend">Added Your Tags</FormLabel>
